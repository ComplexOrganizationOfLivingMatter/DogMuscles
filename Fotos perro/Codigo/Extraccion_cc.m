...
function Extraccion_cc(dapi_fecha,nombre)
...
nombre_2=[nombre ' 10x'];
cd (nombre_2)
nombre_3=[nombre_2 ' P-images'];
cd (nombre_3)

load ('imagenes_necesarias.mat')
load('Datos_plano_azul.mat')


%% OBTENEMOS VECINOS
[H,W]=size(celulas);
aux=celulas;   %%En 'celulas' tenemos las células organizadas como en un voronoi, es decir, con bordes de 1 o 2 píxeles. De aquí obtenemos el nº de vecinos
radio=4;
...
dat=unique(Suma);
grafo_may=dat(1,end);
celulas_validas_no_borde=find(Suma==grafo_may);
celulas_validas_no_borde=celulas_validas_no_borde';
celulas_no_validas_no_borde=(1:(length(num_celulas)));
celulas_no_validas_no_borde(celulas_validas_no_borde)=[];
...
%Area 
%
%Area_celula=regionprops(mascara_mejorada,'Area');
% Area_celula=cat(1,Area_celula.Area);

Area_celula=cell2mat(datos_celulas(:,7));                   %%area celulas de imagen celulas (borde 1px)
...
Mean_R = regionprops(mascara_mejorada, R, 'MeanIntensity');
Mean_R = cat(1, Mean_R.MeanIntensity);
for i=1:max(max(mascara_mejorada))
    Intensidades=R(mascara_mejorada==i);
...
%elongacion
mayor_eje = regionprops(mascara_mejorada,'MajorAxisLength');
mayor_eje = cat(1, mayor_eje.MajorAxisLength);
Mean_mayor_eje = mean(mayor_eje(celulas_validas_no_borde));

menor_eje = regionprops(mascara_mejorada,'MinorAxisLength');
...
%convex hull
Pix_region_convexa=regionprops(mascara_mejorada,'Solidity');
...
angulos = regionprops(mascara_mejorada,'Orientation');
...
centro = regionprops(mascara_mejorada,'Centroid');
centros = cat(1, centro.Centroid);

%Area water

Area_water = cell2mat(datos_celulas_water(:,7));         %%% area de la celula original
% Area_water_ord=zeros(size(Area_water));  %%%Ya lo hemos ordenado en el programa
% for i=1:size(datos_celulas,1)%reordeno atendiendo a la nomenclatura de celula
%     Area_water_ord(i)=Area_water(datos_celulas{i,2}(1,1),1);
% end

%relacion entre areas celulas y las reales del water
% Area_water_validas = Area_water(celulas_validas_no_borde);
Relacion_areas=Area_water./Area_celula;
Relacion_areas_validas=Relacion_areas(celulas_validas_no_borde);
...
aux(aux~=0)=1;
contorno_visual_dilate=bwmorph(1-aux,'dilate');
pi_conotorno=find(contorno_visual_dilate==1);
R_c=ones(H,W);
G_c=ones(H,W);
B_c=ones(H,W);
...
for i=1:size(celulas_validas_no_borde_no_vecinos,1)
    ...
    relacion_Relacion_areas_vecinos(i)=Relacion_areas(numero_celula)/(mean(Relacion_areas(vecinos_network))+std(Relacion_areas(vecinos_network)));
...
for i=1:size(celulas_validas_no_borde,1)
    ...
    if Mean_R(numero_celula)>50 || porc_rojo_intenso(numero_celula)>=0.15
        hold on,
        plot(round(centros(numero_celula,1)),round(centros(numero_celula,2)),'.r','MarkerSize',30)
        ...
        if ~isempty(vecinos_real{numero_celula})
            n_vecinos_rojos_de_rojos(i_rojos)=length(union(find(Mean_R(vecinos_real{numero_celula})>50), find(porc_rojo_intenso(vecinos_real{numero_celula})>=0.15)));
            n_vecinos_negros_de_rojos(i_rojos)=length(intersect(find(Mean_R(vecinos_real{numero_celula})<=50), find(porc_rojo_intenso(vecinos_real{numero_celula})<0.15)));
        end
    else
        
        hold on,
        plot(round(centros(numero_celula,1)),round(centros(numero_celula,2)),'.g','MarkerSize',30)
        
        i_negros=i_negros+1;
        celulas_negras(i_negros)=numero_celula;
        n_vecinos_de_negros(i_negros)=length(vecinos_real{numero_celula});
        if ~isempty(vecinos_real{numero_celula})
            n_vecinos_rojos_de_negros(i_negros)=length(union(find(Mean_R(vecinos_real{numero_celula})>50), find(porc_rojo_intenso(vecinos_real{numero_celula})>=0.15)));
            n_vecinos_negros_de_negros(i_negros)=length(intersect(find(Mean_R(vecinos_real{numero_celula})<=50), find(porc_rojo_intenso(vecinos_real{numero_celula})<0.15)));
        end
    end
...
stringres=strcat(nombre,'_network.bmp');
...
stringres=strcat('Imagenes_procesadas\',fecha,'\',nombre,'\Imagenes_necesarias\', 'Results_69_cc.mat');
...
stringres=strcat('Imagenes_procesadas\',fecha,'\',nombre,'\Imagenes_necesarias\','Datos_cc.mat');
